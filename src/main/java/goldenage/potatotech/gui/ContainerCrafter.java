...
public class ContainerCrafter extends Container {
    public static class SlotCrafterOut extends SlotCrafting {
        public SlotCrafterOut(EntityPlayer entityplayer, IInventory iinventory, IInventory iinventory1, int i, int j, int k) {
            super(entityplayer, iinventory, iinventory1, i, j, k);
        }

        @Override
        public boolean canPutStackInSlot(ItemStack itemstack) {
            return hasStack() && getStack().canStackWith(itemstack);
        }
    ...
    public ContainerCrafter(InventoryPlayer inventoryplayer, TileEntityCrafter tile) {
        if (inventoryplayer == null) {
            return;
        }

        SlotCrafterOut slotCrafting = new SlotCrafterOut(inventoryplayer.player, tile.craftMatrix, tile.craftResult, 0, 124, 35);
